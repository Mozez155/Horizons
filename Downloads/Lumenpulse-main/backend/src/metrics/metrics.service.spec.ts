...
describe('MetricsService', () => {
  ...
  it('recordArticleProcessed() increments horizons_articles_processed_total', async () => {
    service.recordArticleProcessed('coindesk', 'success');
    service.recordArticleProcessed('coindesk', 'duplicate');
    const output = await service.getMetrics();
    expect(output).toContain('horizons_articles_processed_total');
  });

  // Data-pipeline: sentiment //

  it('recordSentimentScore() updates the horizons_sentiment_score gauge', async () => {
    service.recordSentimentScore(0.8, 'coindesk');
    service.recordSentimentScore(0.2, 'coindesk');
    const output = await service.getMetrics();
    expect(output).toContain('horizons_sentiment_score');
  ...
  it('recordModelInference() adds an observation to the histogram', async () => {
    ...
    const output = await service.getMetrics();
    expect(output).toContain('horizons_model_inference_duration_seconds');
  });
  ...
  it('startInferenceTimer() returns a callable that records duration', async () => {
    ...
    const output = await service.getMetrics();
    expect(output).toContain('horizons_model_inference_duration_seconds_sum');
  });

  // Data-pipeline: anomalies //

  it('recordAnomalyDetected() increments horizons_anomalies_detected_total', async () => {
    service.recordAnomalyDetected('price_spike', 'high');
    service.recordAnomalyDetected('sentiment_shift', 'low');
    const output = await service.getMetrics();
    expect(output).toContain('horizons_anomalies_detected_total');
  });

  // Data-pipeline: fetch errors //

  it('recordFetchError() increments horizons_fetch_errors_total', async () => {
    service.recordFetchError('cointelegraph', '429');
    service.recordFetchError('reuters', 'TIMEOUT');
    const output = await service.getMetrics();
    expect(output).toContain('horizons_fetch_errors_total');
  });
