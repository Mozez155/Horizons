...
export function OnboardingProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      try {
        const saved = localStorage.getItem('horizons_onboarding');
        if (saved) {
        ...
        localStorage.removeItem('horizons_onboarding');
      }
    }
  }, []);
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      try {
        localStorage.setItem('horizons_onboarding', JSON.stringify(state));
      } catch {
