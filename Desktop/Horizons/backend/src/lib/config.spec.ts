...
describe('environment-specific loading', () => {
  ...
  it('loads .env.local in development when present', async () => {
    const tempDir = mkdtempSync(path.join(tmpdir(), 'horizons-config-dev-'));
    writeFileSync(
  ...
  it('warns in production when .env file exists and does not throw', async () => {
    const tempDir = mkdtempSync(path.join(tmpdir(), 'horizons-config-prod-'));
    writeFileSync(
    ...
    resetManagedEnv({
      ...
      CORS_ORIGIN: 'https://app.horizons.io',
    });
