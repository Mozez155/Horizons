...
Implements a full-stack project verification system that lets the community vote on which projects earn **Horizons Verified** status and become eligible for matching funds. Voting weight is configurable — flat (one-address-one-vote), reputation-based (from `contributor_registry`), or governance token balance.
